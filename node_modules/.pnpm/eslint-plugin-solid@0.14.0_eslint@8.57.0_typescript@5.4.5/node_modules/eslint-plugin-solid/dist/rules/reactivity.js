...
const { getFunctionHeadLocation } = utils_1.ASTUtils;
...
const getNthDestructuredVar = (id, n, context) => {
    if (id?.type === "ArrayPattern") {
        ...
        if (el?.type === "Identifier") {
            return (0, compat_1.findVariable)(context, el);
        }
    }
    return null;
};
const getReturnedVar = (id, context) => {
    if (id.type === "Identifier") {
        return (0, compat_1.findVariable)(context, id);
    }
    return null;
};
    ...
    create(context, [options]) {
        ...
        const markPropsOnCondition = (node, cb) => {
            if (node.params.length === 1 &&
                ...
                cb(node.params[0])) {
                const propsParam = (0, compat_1.findVariable)(context, node.params[0]);
                if (propsParam) {
            ...
            if (init.type === "CallExpression" && init.callee.type === "Identifier") {
                ...
                if (matchImport(["createSignal", "useTransition"], callee.name)) {
                    ...
                    if ((0, utils_2.isFunctionNode)(arg1) &&
                        ...
                        const indexSignal = (0, compat_1.findVariable)(context, arg1.params[1]);
                        if (indexSignal) {
                    ...
                    if ((0, utils_2.isFunctionNode)(arg1) &&
                        ...
                        const valueSignal = (0, compat_1.findVariable)(context, arg1.params[0]);
                        if (valueSignal) {
        ...
        const checkForTrackedScopes = (node) => {
            ...
            const permissivelyTrackNode = (node) => {
                (0, estraverse_1.traverse)(node, {
                    enter(cn) {
                        const childNode = cn;
                        const traced = (0, utils_2.trace)(childNode, context);
                ...
                if (node.callee.type === "Identifier") {
                    ...
                    if (matchImport([
                        ...
                        if (arg1) {
                            ...
                            const owner = arg0.type === "Identifier" && (0, compat_1.findVariable)(context, arg0);
                            if (owner) {
                    ...
                    if ((0, utils_2.isFunctionNode)(expression)) {
                        ...
                        for (const param of expression.params) {
                            if (param.type === "Identifier" && (0, utils_2.isPropsByName)(param.name)) {
                                const variable = (0, compat_1.findVariable)(context, param);
                                if (variable)
                                    scopeStack.pushProps(variable, currentScope().node);
                            }
                        }
                ...
                if ((0, utils_2.isFunctionNode)(node) &&
                    ...
                    if (element.openingElement.name.type === "JSXIdentifier") {
                        ...
                        if (matchImport("For", tagName) &&
                            ...
                            const index = (0, compat_1.findVariable)(context, node.params[1]);
                            if (index) {
                            ...
                            node.params[0].type === "Identifier") {
                            const item = (0, compat_1.findVariable)(context, node.params[0]);
                            if (item) {
