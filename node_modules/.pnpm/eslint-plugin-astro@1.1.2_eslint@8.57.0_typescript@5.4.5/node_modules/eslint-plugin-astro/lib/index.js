...
function createRule(ruleName, rule) {
  return {
    meta: __spreadProps(__spreadValues({}, rule.meta), {
      docs: __spreadProps(__spreadValues({
        available: () => true
      }, rule.meta.docs), {
...
function getStaticAttributeValue(node, context) {
  var _a2, _b2;
  if (((_a2 = node.value) == null ? void 0 : _a2.type) === types.AST_NODE_TYPES.Literal) {
    return { value: node.value.value };
  }
  if (context && ((_b2 = node.value) == null ? void 0 : _b2.type) === "JSXExpressionContainer" && node.value.expression.type !== "JSXEmptyExpression") {
...
function isTokenOnSameLine(left, right) {
  var _a2, _b2;
  return ((_a2 = left == null ? void 0 : left.loc) == null ? void 0 : _a2.end.line) === ((_b2 = right == null ? void 0 : right.loc) == null ? void 0 : _b2.start.line);
}
      ...
      create(context) {
        ...
        return {
          ImportSpecifier(node) {
            var _a2;
            if (node.imported.name === "getEntryBySlug" && ((_a2 = node.parent) == null ? void 0 : _a2.type) === "ImportDeclaration" && node.parent.source.value === "astro:content") {
              context.report({
                node,
                messageId: "deprecated"
              });
...
var no_exports_from_components_default;
          ...
          context.report({
            ...
            *fix(fixer) {
              ...
              if (element.openingElement.selfClosing) {
                ...
                yield fixer.remove(attr);
                yield* __yieldStar(element.children.map((child) => fixer.remove(child)));
...
function transform(node, context) {
  ...
  try {
    const config = postcssLoadConfig.sync({
      cwd: (_a2 = getCwd(context)) != null ? _a2 : process.cwd(),
      from: filename
    });
    const result = postcss__default.default(config.plugins).process(code, __spreadProps(__spreadValues({}, config.options), {
      map: {
        inline: false
      }
    }));
...
function transformToStyleContentCSS(transform5, context) {
  ...
  return {
    ...
    remap: (index) => {
      outputLocs = outputLocs != null ? outputLocs : new LinesAndColumns(transform5.output);
      inputLocs = inputLocs != null ? inputLocs : new LinesAndColumns(sourceCode.text.slice(...transform5.inputRange));
  ...
  function remapPosition(pos) {
    decoded = decoded != null ? decoded : sourcemapCodec.decode(transform5.mappings);
    const lineMaps = decoded[pos.line - 1];
    if (!(lineMaps == null ? void 0 : lineMaps.length)) {
      for (let line = pos.line - 1; line >= 0; line--) {
        const prevLineMaps = decoded[line];
        if (prevLineMaps == null ? void 0 : prevLineMaps.length) {
...
function parseSelector(selector, context) {
  ...
  return astSelector.nodes.map((sel) => {
    ...
    try {
      ...
      return {
        ...
        offset: (_a2 = sel.sourceIndex) != null ? _a2 : sel.nodes[0].sourceIndex,
        test(element) {
      ...
      if (error instanceof SelectorError) {
        return {
          error,
          selector: sel.toString().trim(),
          offset: (_b2 = sel.sourceIndex) != null ? _b2 : sel.nodes[0].sourceIndex,
          test: () => false
        };
      }
      throw error;
  ...
  function removeGlobals(nodes) {
    ...
    while (nodes[end - 1] && isGlobalPseudo(nodes[end - 1])) {
      ...
      if (((_a2 = nodes[end - 1]) == null ? void 0 : _a2.type) === "combinator") {
        end--;
      }
    }
    while (nodes[start] && isGlobalPseudo(nodes[start])) {
      start++;
      if (((_b2 = nodes[start]) == null ? void 0 : _b2.type) === "combinator") {
        start++;
      }
    }
    if (nodes.some(isRootPseudo)) {
      ...
      if (((_c = nodes[start]) == null ? void 0 : _c.type) === "combinator") {
        start++;
      }
    }
...
function extractClassList(node, context) {
  ...
  if (((_b2 = node.value) == null ? void 0 : _b2.type) === "JSXExpressionContainer" && node.value.expression.type !== "JSXEmptyExpression") {
    const classList = [];
...
function* extractClassListFromExpression(node, context) {
  if (node.type === types.AST_NODE_TYPES.ArrayExpression) {
    for (const element of node.elements) {
      ...
      if (element.type === types.AST_NODE_TYPES.SpreadElement) {
        yield* __yieldStar(extractClassListFromExpression(element.argument, context));
      } else {
        yield* __yieldStar(extractClassListFromExpression(element, context));
      }
    }
    return;
  }
  if (node.type === types.AST_NODE_TYPES.ObjectExpression) {
    for (const prop of node.properties) {
      if (prop.type === types.AST_NODE_TYPES.SpreadElement) {
        yield* __yieldStar(extractClassListFromExpression(prop.argument, context));
        ...
        yield* __yieldStar(extractClassListFromExpression(prop.key, context));
      }
    }
    return;
  }
  ...
  if (staticValue) {
    yield* __yieldStar(extractClassListFromUnknown(staticValue.value));
    return;
  }
  yield null;
}
function* extractClassListFromUnknown(value) {
  ...
  if (Array.isArray(value)) {
    for (const e of value) {
      yield* __yieldStar(extractClassListFromUnknown(e));
    }
    return;
  }
  if (typeof value === "object") {
    yield* __yieldStar(Object.keys(value));
    return;
  }
  yield String(value);
}
  ...
  "src/utils/style/tokenizer.ts"() {
    ...
    CSSTokenizer = class {
      ...
      constructor(text, startOffset, options) {
        ...
        this.text = text;
        this.options = {
          inlineComment: (_a2 = options == null ? void 0 : options.inlineComment) != null ? _a2 : false
        };
        this.cp = NULL;
      ...
      consumeWord(start) {
        ...
        return {
          ...
          range,
          get value() {
            return value != null ? value : value = text.slice(...range);
          }
        };
      }
      /**
      ...
      consumeString(start, quote) {
        ...
        const valueRange = [
          start + 1,
          valueEndOffset != null ? valueEndOffset : this.nextOffset
        ];
        return {
          type: "Quoted" /* quoted */,
          ...
          valueRange,
          get value() {
            return value != null ? value : value = text.slice(...valueRange);
          },
          ...
          function processItems(key, e) {
            if (e.type === "ConditionalExpression") {
              ...
              for (const [expr, str] of sub) {
                result.set(
                  __spreadProps(__spreadValues({}, key), {
                    chains: expr
                  }),
                  str
                );
        ...
        function report(node, map, state) {
          context.report({
            ...
            *fix(fixer) {
              ...
              yield* __yieldStar(state.fixExpression({
                newProps: classProps,
                beforeSpaces,
                afterSpaces,
                node,
        ...
        function verifyAttr(attr) {
          ...
          function findClosestObject(array, target) {
            ...
            for (let index2 = 0; index2 < length; index2++) {
              ...
              if ((after == null ? void 0 : after.type) === types.AST_NODE_TYPES.ObjectExpression) {
                return after;
              }
              const before = beforeElements[index2];
              if ((before == null ? void 0 : before.type) === types.AST_NODE_TYPES.ObjectExpression) {
                return before;
              }
            }
            return null;
                ...
                visitElementExpression(expression2, {
                  ...
                  *fixExpression(data) {
                    ...
                    node.expressions.length > 1) {
                      ...
                      yield* __yieldStar(state.fixArrayElement(data));
                      return;
                    }
                    ...
                    yield* __yieldStar(state.fixExpression(__spreadProps(__spreadValues({}, data), {
              ...
              if (isStringCallExpression(node) && node.arguments[0] && node.arguments[0].type !== types.AST_NODE_TYPES.SpreadElement) {
                visitElementExpression(node.arguments[0], {
                  ...
                  *fixExpression(data) {
                    ...
                    yield* __yieldStar(state.fixExpression(data));
                  }
                });
                  ...
                  *fixExpression(data) {
                    ...
                    if (beforeElements.some((element) => {
                      ...
                      if (beforeElement && isStringLiteral(beforeElement) && afterElement && isStringLiteral(afterElement)) {
                        ...
                        if ((beforeToken == null ? void 0 : beforeToken.value) === "+") {
  ...
  "src/utils/string-literal-parser/tokenizer.ts"() {
    ...
    Tokenizer = class {
      constructor(source, options) {
        ...
        this.end = (_a2 = options.end) != null ? _a2 : null;
        this.ecmaVersion = options.ecmaVersion;
      }
      *parseTokens(quote) {
        var _a2;
        const inTemplate = quote === CP_BACKTICK;
        const endIndex = (_a2 = this.end) != null ? _a2 : this.source.length;
...
function* parseStringTokens(source, option) {
  var _a2, _b2;
  const startIndex = (_a2 = option == null ? void 0 : option.start) != null ? _a2 : 0;
  const ecmaVersion = (_b2 = option == null ? void 0 : option.ecmaVersion) != null ? _b2 : Infinity;
  const tokenizer = new Tokenizer(source, {
    start: startIndex,
    end: option == null ? void 0 : option.end,
    ecmaVersion: ecmaVersion >= 6 && ecmaVersion < 2015 ? ecmaVersion + 2009 : ecmaVersion
  });
  yield* __yieldStar(tokenizer.parseTokens());
}
  ...
  "src/rules/prefer-split-class-list.ts"() {
    ...
    prefer_split_class_list_default = createRule("prefer-split-class-list", {
      ...
      create(context) {
        ...
        const splitLiteral = Boolean((_a2 = context.options[0]) == null ? void 0 : _a2.splitLiteral);
        ...
        function verifyTemplateElement(node, state) {
          ...
          for (const ch of parseStringTokens(sourceCode.text, {
            ...
            if (ch.value.trim()) {
              if (spaces.length) {
                if (shouldReport(__spreadProps(__spreadValues({}, state), { isLeading, isTrailing: false }))) {
                  reportRange([
                    spaces[0].range[0],
          ...
          if (spaces.length) {
            if (shouldReport(__spreadProps(__spreadValues({}, state), { isLeading, isTrailing: true }))) {
          ...
          function reportRange(range) {
            context.report({
              ...
              messageId: "split",
              *fix(fixer) {
                yield* __yieldStar(state.transformArray(fixer));
    ...
    semi_default = createRule("semi", {
      ...
      create(context) {
        ...
        function isEndOfArrowBlock(lastToken) {
          if (!eslintUtils.isClosingBraceToken(lastToken))
            return false;
...
var init_keys = __esm({
  "src/a11y/keys.ts"() {
    ...
    a11yRuleKeys = (plugin == null ? void 0 : plugin.rules) ? Object.keys(plugin.rules).filter(
      (s) => {
        var _a2, _b2, _c;
        return !((_c = (_b2 = (_a2 = plugin == null ? void 0 : plugin.rules) == null ? void 0 : _a2[s]) == null ? void 0 : _b2.meta) == null ? void 0 : _c.deprecated);
      }
    ) : [
      "alt-text",
    ...
    a11yConfigKeys = (plugin == null ? void 0 : plugin.configs) ? Object.keys(plugin.configs) : ["recommended", "strict"];
  }
});
...
${code}
  ...
  parser2.traverseNodes(expression, {
    ...
    enterNode(node) {
      node.loc.start = __spreadProps(__spreadValues({}, node.loc.start), {
        line: node.loc.start.line - 1
      });
      node.loc.end = __spreadProps(__spreadValues({}, node.loc.end), {
  ...
  "src/shared/client-script/index.ts"() {
    ...
    ClientScript = class {
      ...
      initBlock() {
        ...
        const lines = this.code.slice(startOffset, endOffset).split(new RegExp("(?<=\\n)", "u"));
...
`);
      ...
      extractDefineVars() {
        ...
        if (expression.type !== types.AST_NODE_TYPES.ObjectExpression)
          return [];
...
var init_processor = __esm({
  "src/processor/index.ts"() {
    ...
    clientSideTsProcessor = {
      ...
      postprocess,
      supportsAutofix: true,
      meta: __spreadProps(__spreadValues({}, meta_exports), { name: "astro/client-side-ts" })
    };
  }
});
...
var plugin3, _a, _b, base_default;
    ...
    base_default = [
      {
        plugins: {
          get astro() {
            return plugin3 != null ? plugin3 : plugin3 = (init_plugin(), __toCommonJS(plugin_exports)).plugin;
          }
        }
      },
      {
        files: ["*.astro", "**/*.astro"],
        languageOptions: {
          globals: __spreadValues(__spreadValues({}, globals__default.default.node), environments.astro.globals),
          parser: parser2__namespace,
          // The script of Astro components uses ESM.
          sourceType: "module",
          parserOptions: {
            parser: (_a = tsESLintParser) != null ? _a : void 0,
            extraFileExtensions: [".astro"]
          }
        },
        rules: {
      ...
      },
      {
        ...
        languageOptions: {
          globals: __spreadValues({}, globals__default.default.browser),
          parser: (_b = tsESLintParser) != null ? _b : void 0,
          sourceType: "module",
          parserOptions: {
            project: null
          }
  ...
  for (const configName of a11yConfigKeys) {
    Object.defineProperty(configs, `jsx-a11y-${configName}`, {
      ...
      get() {
        ...
        const baseConfig = (_b2 = (_a2 = base == null ? void 0 : base.configs) == null ? void 0 : _a2[configName]) != null ? _b2 : {};
        const baseRules = (_c = baseConfig.rules) != null ? _c : {};
        const newRules = {};
...
function getPluginJsxA11yRule(ruleName) {
  var _a2;
  const base = getPluginJsxA11y();
  return (_a2 = base == null ? void 0 : base.rules) == null ? void 0 : _a2[ruleName];
}
function buildRules() {
  const rules3 = [];
  for (const ruleKey of a11yRuleKeys) {
    ...
    const docs = __spreadProps(__spreadValues({}, ruleWithoutMeta.meta.docs), {
      extensionRule: {
        plugin: "eslint-plugin-jsx-a11y",
        get url() {
          var _a2, _b2, _c, _d;
          return (_d = (_c = (_b2 = (_a2 = getPluginJsxA11yRule(ruleKey)) == null ? void 0 : _a2.meta) == null ? void 0 : _b2.docs) == null ? void 0 : _c.url) != null ? _d : `https://github.com/jsx-eslint/eslint-plugin-jsx-a11y/blob/main/docs/rules/${ruleKey}.md`;
        }
      }
    });
    const newRule = {
      meta: new Proxy(ruleWithoutMeta.meta, {
        get(_t, key) {
          ...
          const baseRule = getPluginJsxA11yRule(ruleKey);
          return (_b2 = (_a2 = baseRule == null ? void 0 : baseRule.meta) == null ? void 0 : _a2[key]) != null ? _b2 : ruleWithoutMeta.meta[key];
        }
      }),
...
var init_all = __esm({
  "src/configs/all.ts"() {
    ...
    all_default = {
      ...
      rules: __spreadValues(__spreadValues({}, all), recommended_default.rules)
    };
  }
});
...
var init_all2 = __esm({
  "src/configs/flat/all.ts"() {
    ...
    all_default2 = [
      ...recommended_default2,
      {
        rules: __spreadValues(__spreadValues({}, all2), recommended_default2[recommended_default2.length - 1].rules)
      }
    ];
  }
});
